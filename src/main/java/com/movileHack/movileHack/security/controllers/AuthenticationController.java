package com.movileHack.movileHack.security.controllers;
...
import com.movileHack.movileHack.security.dto.TokenDto;
...
public class AuthenticationController {
	...
	public ResponseEntity<Response<TokenDto>> gerarTokenJwt(@Valid @RequestBody JwtAuthenticationDto authenticationDto, BindingResult result) throws AuthenticationException {
		...
		try {
			...
			boolean userLogado = usuarioService.login(authenticationDto);
			if (userLogado) {
				UserDetails userDetails = userDetailsService.loadUserByUsername(authenticationDto.getEmail());
				String token = jwtTokenUtil.obterToken(userDetails);
				response.setData(new TokenDto(token));
			}else {
				result.getAllErrors().forEach(error -> response.getErrors().add("Senha incorreta"));
				return ResponseEntity.badRequest().body(response);
			}
		} catch (Exception e) {
			result.getAllErrors().forEach(error -> response.getErrors().add(error.getDefaultMessage()));
